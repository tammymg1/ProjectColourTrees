...
class SQtree {
 ...
 private:
  ...
  Node *copyRecursion(Node * other, Node * root);
  
  pair<int, int> findDivision(stats & s, pair<int, int> & ul, int w, int h);

  void renderRecursion(PNG & p, Node * root);

  void sizeRecursion(int &num, Node * root);
